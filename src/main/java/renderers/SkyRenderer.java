...
public class SkyRenderer {
    ...
    public void renderSkybox() {
        ...
        sky = lerpColor(sky, twilightColor, twilightStrength);
        float r = sky[0];
        float g = sky[1];
        float b = sky[2];
    ...
    public float[] getLightColor() {
        ...
        if (brightness > 0f) {
            ...
            float duskWeight = total > 0f ? sunset / total : 0f;
            float[] twilightColor = new float[] {
                    1.3f * dawnWeight + 1.2f * duskWeight,
                    0.75f * dawnWeight + 0.55f * duskWeight,
                    0.4f * dawnWeight + 0.7f * duskWeight
            };
            float[] sunColor = lerpColor(day, twilightColor, twilightStrength);
            float intensity = Math.max(0.15f, brightness);
