...
public class Chunk {
    ...
    private static void applyCavesAndRavines(float[][] heights, float[][] riverSurface, int cx, int cz, float scale,
        ...
        final double riverWidthJitter = 0.02;
        final double riverDepthJitter = 1.4;
            ...
            for (int z = 0; z <= SIZE; z++) {
                ...
                if (riverBand < riverBlendWidth) {
                    ...
                    if (Math.abs(riverMask) < riverMaskThreshold) {
                        ...
                        double widthNoise = terrainNoise.eval(wx * 0.004 + 420.0, wz * 0.004 - 220.0);
                        double depthNoise = terrainNoise.eval(wx * 0.003 + 820.0, wz * 0.003 - 120.0);
                        double localWidth = Math.max(0.01, riverWidth + widthNoise * riverWidthJitter);
                        double localDepth = Math.max(0.5, riverDepth + depthNoise * riverDepthJitter);
                        ...
                        float surfaceOffset = (float) (terrainNoise.eval(wx * 0.002 + 1200.0, wz * 0.002 - 800.0) * 0.6);
                        float riverSurfaceHeight = baseHeight - 0.6f + surfaceOffset;
                        float target = baseHeight - (float) (localDepth * depthFactor);
                        float blended = (float) (baseHeight * (1.0 - bankBlend) + target * bankBlend);
                        heights[x][z] = blended;
                        if (riverSurface != null && t > 0.0) {
