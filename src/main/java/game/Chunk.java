...
public class Chunk {
    ...
    public void drawTerrainAndFeatures(int chunkDistance, int featureNearDistance, int featureMidDistance,
        ...
        boolean simplified = chunkDistance > featureNearDistance;
        boolean farTier = chunkDistance > featureMidDistance;
        // Draw features if they are above water
        for (Feature f : features) {
            if (f instanceof Grass) {
                continue;
            }
            if (farTier && !isFarFeature(f)) {
                continue;
            }
    ...
    private void buildTerrainBuffers() {
        ...
        addSkirts(builders, texScale, step);
    ...
    public void renderDepth(int chunkDistance, int featureDetailDistance, int grassDetailDistance) {
        ...
        if (chunkDistance > featureDetailDistance + 4) {
            return;
        }
        boolean simplified = chunkDistance > featureDetailDistance;
        boolean farTier = chunkDistance > featureDetailDistance + 2;
        for (Feature f : features) {
            if (f instanceof Grass) {
                continue;
            }
    ...
    private void addSkirts(Map<Integer, FloatBuilder> builders, float texScale, int step) {
        for (int x = 0; x < SIZE; x += step) {
            if (x + step > SIZE) {
                continue;
            }
            addSkirtQuad(builders, x, 0, x + step, 0, heights[x][0], heights[x + step][0],
                    0f, 0f, -1f, texScale);
            addSkirtQuad(builders, x, SIZE, x + step, SIZE, heights[x][SIZE], heights[x + step][SIZE],
                    0f, 0f, 1f, texScale);
        }
        ...
        for (int z = 0; z < SIZE; z += step) {
            ...
            addSkirtQuad(builders, 0, z, 0, z + step, heights[0][z], heights[0][z + step],
                    -1f, 0f, 0f, texScale);
            addSkirtQuad(builders, SIZE, z, SIZE, z + step, heights[SIZE][z], heights[SIZE][z + step],
                    1f, 0f, 0f, texScale);
        }
    }

    private void addSkirtQuad(Map<Integer, FloatBuilder> builders, int x1, int z1, int x2, int z2,
                               float y1, float y2, float nx, float ny, float nz, float texScale) {
        float height = Math.max(y1, y2);
        int tex = pickTexture(height, 1f);
        FloatBuilder builder = builders.computeIfAbsent(tex, key -> new FloatBuilder());
        ...
        builder.putVertex(wx1, y1, wz1, normal, x1 * texScale, z1 * texScale);
        builder.putVertex(wx2, y2, wz2, normal, x2 * texScale, z2 * texScale);
        builder.putVertex(wx2, y2b, wz2, normal, x2 * texScale, z2 * texScale + texScale);

        builder.putVertex(wx1, y1, wz1, normal, x1 * texScale, z1 * texScale);
        builder.putVertex(wx2, y2b, wz2, normal, x2 * texScale, z2 * texScale + texScale);
        builder.putVertex(wx1, y1b, wz1, normal, x1 * texScale, z1 * texScale + texScale);
    }

    private boolean isFarFeature(Feature feature) {
        return feature instanceof Tree || feature instanceof Cactus;
    }
