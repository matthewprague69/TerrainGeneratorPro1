...
public class Chunk {
    ...
    private float minChunkHeight = 0f;
    private float maxChunkHeight = 0f;
    ...
    public static class ChunkBuildData {
        ...
        public ChunkBuildData(int cx, int cz, int lod, Biome biome, float[][] heights, boolean[][] featureMask,
                              boolean[][] lakeMask, List<Feature> lakes) {
            this.cx = cx;
            this.cz = cz;
            this.lod = lod;
            this.biome = biome;
    ...
    public FeatureGenerationInput createFeatureGenerationInput() {
        return new FeatureGenerationInput(cx, cz, scale, biome, heights,
    ...
    private float getSnowBaseHeightAtVertex(int x, int z, boolean frozenWater, float iceThickness) {
        ...
        if (!frozenWater || base >= WATER_LEVEL) {
            return base;
        }

        float frozenSurface = WATER_LEVEL + Math.max(0f, iceThickness);
    ...
    private float getSnowDepthForVertex(int x, int z, float snowCoverage) {
        ...
        if (manager.isWaterFrozen() && heights[x][z] < WATER_LEVEL) {
    ...
    private void buildWaterGeometry() {
        ...
        for (int z = 0; z < SIZE; ) {
            ...
            int z2 = Math.min(z + stepZ, SIZE);
            for (int x = 0; x < SIZE; ) {
                int stepX = (x < edgeBand || x >= SIZE - edgeBand) ? 1 : baseStep;
                int x2 = Math.min(x + stepX, SIZE);
    ...
    private void ensureWaterSimInitialized() {
        ...
        if (waterSimDirty) {
            waterSimChunk.initializeFromTerrainAndSurface(heights, WATER_LEVEL);
            waterSimDirty = false;
        }
    }
    ...
    private void renderWaterSurface(float timeSeconds, boolean frozen, int chunkDistance, float cameraWx, float cameraWz) {
        ...
        boolean simplifyShading = !frozen && chunkDistance >= 14;
        ...
        for (WaterPatch patch : waterPatches) {
            ...
            float disturbance1 = waterSimChunk.sampleDisturbance((patch.wx1 / scale) - (cx * SIZE), (patch.wz1 / scale) - (cz * SIZE));
            float disturbance2 = waterSimChunk.sampleDisturbance((patch.wx2 / scale) - (cx * SIZE), (patch.wz1 / scale) - (cz * SIZE));
            float disturbance3 = waterSimChunk.sampleDisturbance((patch.wx2 / scale) - (cx * SIZE), (patch.wz2 / scale) - (cz * SIZE));
            float disturbance4 = waterSimChunk.sampleDisturbance((patch.wx1 / scale) - (cx * SIZE), (patch.wz2 / scale) - (cz * SIZE));
            ...
            if (wet1 && wet2 && wet3) {
                submitWaterVertex(patch.wx1, patch.wz1, wy1, surface1, patch.terrainY1,
                        timeSeconds, frozen, lx, ly, lz, texScale, waveScale1, dx1, dz1, curvature, disturbance1, simplifyShading);
                submitWaterVertex(patch.wx2, patch.wz1, wy2, surface2, patch.terrainY2,
                        timeSeconds, frozen, lx, ly, lz, texScale, waveScale2, dx2, dz2, curvature, disturbance2, simplifyShading);
                submitWaterVertex(patch.wx2, patch.wz2, wy3, surface3, patch.terrainY3,
                        timeSeconds, frozen, lx, ly, lz, texScale, waveScale3, dx3, dz3, curvature, disturbance3, simplifyShading);
            }
            // Triangle 2: (1,3,4)
            if (wet1 && wet3 && wet4) {
                submitWaterVertex(patch.wx1, patch.wz1, wy1, surface1, patch.terrainY1,
                ...
                submitWaterVertex(patch.wx1, patch.wz2, wy4, surface4, patch.terrainY4,
                        timeSeconds, frozen, lx, ly, lz, texScale, waveScale4, dx4, dz4, curvature, disturbance4, simplifyShading);
            }
        }
        glEnd();
    }
    ...
    private void submitWaterVertex(float wx, float wz, float wy, float baseY, float terrainY,
                                   ...
                                   float waveScale, float dx, float dz, float curvature, float disturbance,
                                   boolean simplifyShading) {
        if (frozen) {
            ...
            if (simplifyShading) {
                float deepBlend = smoothstep01(depth01);
                float flatR = lerp(0.08f, 0.12f, deepBlend);
                float flatG = lerp(0.30f, 0.40f, deepBlend);
                float flatB = lerp(0.54f, 0.66f, deepBlend);
                float fresnel = clamp01((1f - ny) * 1.1f + slopeMag * 0.18f);
                float brightness = Math.min(1f, 0.50f + diffuse * 0.38f);
                float litR = clamp01(flatR * (0.78f + brightness * 0.42f) + fresnel * 0.05f);
                float litG = clamp01(flatG * (0.78f + brightness * 0.44f) + fresnel * 0.06f);
                float litB = clamp01(flatB * (0.80f + brightness * 0.46f) + fresnel * 0.08f);
                float foamInfluence = clamp01(disturbance * 0.25f);
                float finalR = litR + (1f - litR) * foamInfluence * 0.55f;
                float finalG = litG + (1f - litG) * foamInfluence * 0.65f;
                float finalB = litB + (1f - litB) * foamInfluence * 0.75f;
                float alpha = Math.min(0.97f, Math.max(0.88f, 0.84f + shorelineMix * 0.08f + fresnel * 0.07f));
            ...
            float rippleFoam = clamp01(disturbance * (0.38f + slopeMag * 0.95f + (1f - depthSmooth) * 0.32f));
            foam = Math.max(foam, rippleFoam);
            ...
            float rippleWhiten = clamp01(disturbance * (0.30f + 0.50f * (1f - depthSmooth)));
            float finalR = litR + (1f - litR) * Math.max(foamMix, rippleWhiten * 0.72f);
            float finalG = litG + (1f - litG) * Math.max(foamMix, rippleWhiten * 0.78f);
            float finalB = litB + (1f - litB) * Math.max(foamMix, rippleWhiten * 0.84f);
            ...
            float alpha = Math.min(0.99f,
                    Math.max(0.90f, 0.79f + shorelineMix * 0.16f + foam * 0.20f + fresnel * 0.10f + sideOpacityBoost * 0.10f + disturbance * 0.05f));
            glColor4f(finalR, finalG, finalB, alpha);
        }
    ...
    private float getWaveAttenuationAtDistance(float wx, float wz, float cameraWx, float cameraWz) {
        float chunkWorldSpan = SIZE * scale;
        float dxChunks = (wx - cameraWx) / chunkWorldSpan;
        float dzChunks = (wz - cameraWz) / chunkWorldSpan;
        float distanceChunks = (float) Math.sqrt(dxChunks * dxChunks + dzChunks * dzChunks);

        final float fullDetailDistanceChunks = 14.0f;
        final float flatWaterDistanceChunks = 21.0f;
        if (distanceChunks <= fullDetailDistanceChunks) {
            return 1f;
        }
        if (distanceChunks >= flatWaterDistanceChunks) {
            return 0f;
        }

        float t = (distanceChunks - fullDetailDistanceChunks) / (flatWaterDistanceChunks - fullDetailDistanceChunks);
    ...
    private void generateFeatures() {
        FeatureGenerationResult result = generateFeatureSpawns(
                new FeatureGenerationInput(cx, cz, scale, biome, heights, featureMask, lakeMask,
                        manager.getSeed()));
    ...
    public void markRenderDirty() {
        renderResourcesBuilt = false;
        // Keep the previous simulation allocated so already-rendered water stays visually stable
        // while replacement LOD/render resources are queued.
        waterSimDirty = true;
    }
    ...
    public float getMinChunkHeight() {
    ...
    private void recomputeHeightBounds() {
        ...
        if (!Float.isFinite(minH) || !Float.isFinite(maxH)) {
            minH = -200f;
            maxH = 1000f;
        }

        float waterCeiling = WATER_LEVEL + WATER_WAVE_AMPLITUDE * WATER_LARGE_SWELL_MULTIPLIER + 1.5f;
        minChunkHeight = Math.min(minH, WATER_LEVEL - 6f);
        maxChunkHeight = Math.max(maxH, waterCeiling);
    }
