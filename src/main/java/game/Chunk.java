...
public class Chunk {
    ...
    private ImpostorSize getImpostorSize(Feature feature, float heightBucket, float canopyBucket,
        ...
        if (feature instanceof Tree) {
            width = Math.max(canopyBucket * 1.8f, heightBucket * 0.2f);
            height = heightBucket + canopyBucket;
    ...
    private ImpostorEntry getImpostorEntry(Feature feature) {
        ...
        if (feature instanceof Tree) {
            ...
            heightBucket = Math.max(0.25f, quantizeUp(tree.getFullHeight(), 0.25f));
    ...
    private int renderImpostorTexture(Feature feature, ImpostorSize size) {
        ...
        boolean impostorTree = feature instanceof Tree;
        if (impostorTree) {
            Tree.setImpostorRendering(true);
        }
        feature.draw();
        if (impostorTree) {
            Tree.setImpostorRendering(false);
        }
        glPopMatrix();
