...
/*EVEN BIOMES NOT FINNISHED
    ...
    public void drawTerrainAndFeatures(float wx, float wz) {
        ...
        int pcx = (int) Math.floor(wx / (Chunk.SIZE * scale));
        int pcz = (int) Math.floor(wz / (Chunk.SIZE * scale));
        int featureNearDistance = Math.max(1, featureRenderDist);
        int featureMidDistance = Math.max(featureNearDistance + 1, featureRenderDist + 2);
        int featureFarDistance = Math.max(featureMidDistance + 1, featureRenderDist + 4);
